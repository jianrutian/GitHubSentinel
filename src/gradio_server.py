...
subscription_manager = SubscriptionManager(config.github_subscriptions_file)


def export_hacker_news_report(items_type, items_num):
    # 定义一个函数，用于导出和生成指定时间范围内项目的进展报告
    raw_file_path = hacker_news_client.export_daily_items(items_type, items_num)  # 导出原始数据文件路径
    report, report_file_path = report_generator.generate_report("hacker_news", raw_file_path)  # 生成并获取报告内容及文件路径

    return report, report_file_path  # 返回报告内容和报告文件路径


def export_github_report(repo, days):
    # 定义一个函数，用于导出和生成指定时间范围内项目的进展报告
    raw_file_path = github_client.export_progress_by_date_range(repo, days)  # 导出原始数据文件路径
    report, report_file_path = report_generator.generate_report("github", raw_file_path)  # 生成并获取报告内容及文件路径
...
with gr.Blocks(title="GitHubSentinel", theme=gr.themes.Monochrome()) as demo:
    ...
    with gr.Tab("生成报告"):
        with gr.Row():
            with gr.Column():
                ...
                github_button = gr.Button("提交")

                items_type = gr.Dropdown(
                    config.hn_items_types, label="hacker news类型列表", info="hacker news"
                )  # 下拉菜单选择订阅的GitHub项目
                items_nums = gr.Slider(value=10, minimum=5, maximum=30, step=1, label="items数量",
                                 info="想要获取得items数量，单位：条")
    ...
    github_button.click(export_github_report, inputs=[repo, days], outputs=[report, report_file_path])
    hacker_button.click(export_hacker_news_report, inputs=[items_type, items_nums], outputs=[report, report_file_path])
